 ...
 * 0.002 at 24 kHz → HPF corner ≈ 0.002 × 24000 / (2π) ≈ 7.6 Hz. */
...
void flux_est_update(FluxEst_t *est, float e_alpha, float e_beta, float Ts)
{
     ...
     * psi = (1 - alpha) × (psi_prev + e_bemf × Ts)
     ...
     * For a PMSM: flux_alpha = Lm × cos(theta), flux_beta = Lm × sin(theta)
     * Back-EMF = d(flux)/dt, so integrated BEMF = flux (shifted by 90°).
     * theta = atan2(-psi_alpha, psi_beta) corrects the 90° phase. */
