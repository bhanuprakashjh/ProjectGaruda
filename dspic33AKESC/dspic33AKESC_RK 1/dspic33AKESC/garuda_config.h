 ...
 * 1 = Internal op-amps (OA1/OA2/OA3 on dsPIC33AK DIM module, OMONEN→ADC)
 * 0 = External op-amps (on MCLV-48V-300W board, signal read via ADC pins)
 * Both modes use same ADC channels, same ×8 gain, same CURRENT_SCALE.
 * Ensure board jumpers match this setting (see board user guide DS50003297). */
#define USE_INTERNAL_OPAMP       0

/* Feature Flags (0=disabled, 1=enabled) */
/* FOC conversion: all 6-step BEMF/commutation features disabled */
#define FEATURE_BEMF_CLOSED_LOOP 0  /* Disabled: FOC does not use BEMF ZC */
#define FEATURE_VBUS_FAULT       0  /* Disabled: FOC handles voltage via soft limits */
#define FEATURE_DESYNC_RECOVERY  0  /* Disabled: FOC uses stall fault instead */
#define FEATURE_DUTY_SLEW        0  /* Disabled: FOC current loop handles slew */
#define FEATURE_TIMING_ADVANCE   0  /* Disabled: FOC uses PLL angle directly */
#define FEATURE_DYNAMIC_BLANKING 0  /* Disabled: not applicable to FOC */
#define FEATURE_VBUS_SAG_LIMIT   0  /* Disabled: FOC SVPWM handles Vbus naturally */
#define FEATURE_BEMF_INTEGRATION 0  /* Disabled: replaced by back-EMF observer */
#define FEATURE_SINE_STARTUP     0  /* Disabled: FOC uses I/F open-loop ramp */
#define FEATURE_ADC_CMP_ZC       0  /* Disabled: not applicable to FOC */
#define FEATURE_HW_OVERCURRENT   0  /* Disabled: OA3 now used for phase C current */
...
#define FEATURE_SMO              1  /* Sliding Mode Observer (parallel to voltage-model) */
