 ...
 *   2. SetupGPIOPorts() — PWM, current-sense, LED, button, UART pins
...
int main(void)
{
    ...
    InitOscillator();I_alpha= I_beta=0;
    ...
    /* Internal op-amp mode: enable OA1/OA2/OA3 for 3-phase current sensing.
     * External op-amp mode: board's op-amps feed the same ADC pins — skip init.
     * Controlled by USE_INTERNAL_OPAMP in garuda_config.h. */
#if USE_INTERNAL_OPAMP
    HAL_OA3_Init();
#endif
    ...
    while (1)
    {
        ...
        /* ── LED2 State Diagnostic ──────────────────────────────────────
         * Without WebSerial, LED2 shows which ESC state is active:
         *   IDLE:    OFF
         *   ARMED:   Fast blink (~5 Hz)   — SW1 registered, arming
         *   STARTUP: Very fast blink (~20 Hz) — open-loop ramp
         *   RUNNING: Solid ON
         *   FAULT:   Slow blink (~1 Hz)
         * ──────────────────────────────────────────────────────────────── */
        {
            uint32_t tick = garudaData.systemTick;
            switch (garudaData.state)
            {
                case ESC_IDLE:
                    ...
                    break;
                case ESC_ARMED:
                    LED2 = (tick / 100) & 1;    /* toggle every 100ms */
                    break;
                case ESC_STARTUP:
                    LED2 = (tick / 25) & 1;     /* toggle every 25ms */
                    break;
                case ESC_RUNNING:
                    LED2 = 1;
                    break;
                case ESC_FAULT:
                {
                    /* Blink-count encodes fault code in a 2-second cycle:
                     *   1 blink = OVERVOLTAGE
                     *   2 blinks = UNDERVOLTAGE
                     *   3 blinks = OVERCURRENT (software)
                     *   4 blinks = BOARD_PCI (FPCI hardware)
                     *   5 blinks = STALL
                     * Each blink = 100ms ON + 100ms OFF.  Then LED off until 2s. */
                    uint32_t cycle = tick % 2000U;
                    uint32_t blinkEnd = (uint32_t)garudaData.faultCode * 200U;
                    if (cycle < blinkEnd)
                        LED2 = (cycle / 100U) & 1;
                    else
                        LED2 = 0;
