...
/* GSP_SNAPSHOT_T — 92 bytes, returned by GET_SNAPSHOT */
typedef struct __attribute__((packed)) {
    /* Core state (8B) */
    uint8_t  state;
    uint8_t  faultCode;
    ...
    /* FOC telemetry (28B) — appended for backward compat */
    float    focId;             /* D-axis current (A) */
    float    focIq;             /* Q-axis current (A) */
    float    focTheta;          /* Rotor angle (rad, 0–2π) — primary estimator */
    float    focOmega;          /* Rotor speed (rad/s electrical) */
    float    focVbus;           /* Bus voltage (V) */
    float    focIdc;            /* Estimated DC bus current (A) */
    float    focTheta2;         /* Flux estimator angle (rad, 0–2π) — parallel */
} GSP_SNAPSHOT_T;

_Static_assert(sizeof(GSP_SNAPSHOT_T) == 96, "GSP_SNAPSHOT_T wire size mismatch");
