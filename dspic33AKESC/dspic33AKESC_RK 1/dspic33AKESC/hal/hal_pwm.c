 ...
 * @file hal_pwm.c
 *
 * @brief PWM module configuration for FOC SVPWM.
 * Adapted from 6-step version:
 *   - Keeps: PCLKCON, MPER, dead time, complementary mode, PG1 master,
 *            PG2+PG3 slave, bootstrap charging, ADC trigger on PG1TRIGA
 *   - Removes: 6-step commutation table, SetCommutationStep
 *   - Adds: HAL_PWM_SetDutyFloat3Phase() for FOC normalized duties [0,1]
 *
...
/**
 * @brief Set independent duty cycles on all three PWM generators (FOC).
 * Each phase gets its own duty count. Clamps to MIN_DUTY..MAX_DUTY.
...
/**
 * @brief Set SVPWM duties from normalized floats [0.0, 1.0].
 * Converts each float duty to the PWM count range [MIN_DUTY, MAX_DUTY]
 * and writes all three generators atomically.
 * @param da  Phase A duty [0.0, 1.0]
 * @param db  Phase B duty [0.0, 1.0]
 * @param dc  Phase C duty [0.0, 1.0]
 */
void HAL_PWM_SetDutyFloat3Phase(float da, float db, float dc)
{
    uint32_t range = (uint32_t)(MAX_DUTY - MIN_DUTY);

    if (da < 0.0f) da = 0.0f; if (da > 1.0f) da = 1.0f;
    if (db < 0.0f) db = 0.0f; if (db > 1.0f) db = 1.0f;
    if (dc < 0.0f) dc = 0.0f; if (dc > 1.0f) dc = 1.0f;

    uint32_t cntA = MIN_DUTY + (uint32_t)(da * (float)range);
    uint32_t cntB = MIN_DUTY + (uint32_t)(db * (float)range);
    uint32_t cntC = MIN_DUTY + (uint32_t)(dc * (float)range);

    /* Write slaves first, then master (master update broadcasts to slaves) */
    PWM_PDC3 = cntC;
    PWM_PDC2 = cntB;
    PWM_PDC1 = cntA;
}
...
/**
 * @brief Release all PWM override registers (allow PWM generators to drive pins).
 * Sets OVRENH=0, OVRENL=0 on PG1, PG2, PG3. Called once at startup and
 * at ESC_STARTUP/ESC_RUNNING entry.
 */
